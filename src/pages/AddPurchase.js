...
export default class AddPurchase extends Component {
  ...
  componentWillMount() {
    ...
    db.collection("purchase")
      ...
      .then(snapshot => {
        ...
        var Uniqueid = purchase.length + 1;

        for (var i = 0; i < purchase.length; i++) {
          var prevSaleId = Number(purchase[i].purchase_idd);
          if (Uniqueid === prevSaleId) {
            Uniqueid++;
          }
          this.setState({
            v_purchase_id: Uniqueid.toString()
          });
        }
      })
      .catch(error => {
        alert("oops , cant load the Purchase record");
      });
  }

  getPurchaseId = e => {
    this.setState({
      v_purchase_id: e.target.value
    });
  };
  getDiscount = e => {
    this.setState({
  ...
  AddValues = () => {
    ...
    const db = firebase.firestore();
    const purchase_id = this.state.v_purchase_id;
    ...
    const creditor_name = this.state.v_creditor_name;
    ...
    if (this.state.v_creditor_name !== "") {
      db.collection("AccountPayable").add({
        purchase_id: purchase_id,
        pro_name: pro_name,
        quantity: quantity,
        total_price: total_price,
        ...
        <form>
          <div className="from-row">
            <div className="col-md-3 ml-3 mb-3 mb-form">
              <label htmlFor="validationDefault22">Purchase id</label>
              <input
                type="text"
                onChange={this.getPurchaseId}
                defaultValue={this.state.v_purchase_id}
